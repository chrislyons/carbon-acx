...
_DISCLOSURE_SENTENCES = (
    ...
    "The Dash preview and Cloudflare Pages client both read immutable precomputed bundles under ``dist/artifacts`` generated by ``make build``, including the Plotly JSON under ``dist/artifacts/figures/`` and IEEE references under ``dist/artifacts/references/``.",
    "No live data collection or API calls occur at runtime.",
    "Refer to ``dist/artifacts/manifest.json`` for the snapshot timestamp and scope of coverage.",
)
