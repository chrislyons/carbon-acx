...
def create_app() -> Dash:
    ...
    def _summarise_layers(
        ...
        emission_years = (
            metadata.get("emission_years") if isinstance(metadata, Mapping) else []
        )
        methods = metadata.get("methods") if isinstance(metadata, Mapping) else []
        grid_sources = (
            metadata.get("grid_sources") if isinstance(metadata, Mapping) else []
        )
    ...
    def _build_share_payload(
        ...
        ordered_layers = _active_layers_for(
            selected_layers, view_mode_value, available_candidates
        )
        ...
        layer_text = _format_values(
            summary["layer_labels"] or ["None selected"], "None selected"
        )
        caption = " • ".join(
            [
                f"Scope: {scope_text}",
    ...
    def _sync_state_from_url(search_value, hash_value, available_layers_state, default_layers_state):
        ...
        ordered_layers = _active_layers_for(
            layer_values, view_mode, available or fallback
        )
    ...
    def _update_badges(selected_layers, view_mode, metadata_store, available_layers):
        ordered_layers = _active_layers_for(
            selected_layers, view_mode, available_layers if isinstance(available_layers, list) else []
        )
        summary = _summarise_layers(ordered_layers, metadata_store)
        emission_years = summary["years"] or summary["emission_years"]
        layer_labels = summary["layer_labels"] or [
            _layer_label(layer) for layer in ordered_layers
        ]
