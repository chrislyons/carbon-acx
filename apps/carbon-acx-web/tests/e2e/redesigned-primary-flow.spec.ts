...
test('Home numeric fallback remains functional without optional pointer APIs', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    Object.defineProperty(SVGElement.prototype, 'releasePointerCapture', { configurable: true, value: undefined })
...
test('Home graph controls overlay a viewport-responsive plot', async ({ page }) => {
  for (const viewport of [{ width: 1568, height: 1018 }, { width: 1032, height: 1568 }]) {
    ...
    expect(metrics.valueBottom).toBeLessThanOrEqual(metrics.rowBottom + 0.5)
    expect(metrics.quantityTop).toBeGreaterThanOrEqual(metrics.rowTop - 0.5)
    expect(metrics.quantityBottom).toBeLessThanOrEqual(metrics.rowBottom + 0.5)
